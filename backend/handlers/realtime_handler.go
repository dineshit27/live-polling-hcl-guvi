...
// StreamWorkspaceEvents handles GET /api/workspace/events
// Establishes a global SSE stream that fans out all poll events (votes, status changes, etc.)
// to the Live Polling Workspace dashboard. Subscribes to the global Redis "polls:events" channel.
func (h *RealtimeHandler) StreamWorkspaceEvents(c *gin.Context) {
	if h.subscriber == nil {
		RespondError(c, http.StatusServiceUnavailable, "realtime_unavailable", "Realtime service is currently unavailable")
		return
	}
	...
	// Subscribe to the global polls:events Redis channel
	ctx, cancel := context.WithCancel(c.Request.Context())
	defer cancel()

	payloadChan, cleanup, err := h.subscriber.SubscribeAll(ctx)
	if err != nil {
		_ = sendSSEComment(c, fmt.Sprintf("subscription error: %v", err))
		return
	}
	...
	// Send an initial CONNECTED event so the browser EventSource immediately fires onopen and updates state
	_ = sendSSE(c, "CONNECTED", map[string]any{
		"status":    "connected",
		"timestamp": time.Now().UTC(),
	})

	// Heartbeat ticker to keep alive through proxies
	ticker := time.NewTicker(15 * time.Second)
	defer ticker.Stop()
