...
export const PollCard: React.FC<PollCardProps> = ({
  ...
  const leadingOption =
    poll.total_votes > 0 && Array.isArray(poll.options)
      ? [...poll.options].sort((a, b) => b.vote_count - a.vote_count)[0]
      : null;

  // Always use a safe options array to prevent iteration crashes from malformed API responses
      ...
      <div className="mt-4 pt-3 border-t border-slate-800/80">
        ...
        <div className="flex items-center justify-between gap-4 mt-3 text-xs text-slate-400 font-mono">
          ...
          <div className="flex items-center gap-3 text-[11px] text-slate-500">
            <span>{safeOptions.length} options</span>
