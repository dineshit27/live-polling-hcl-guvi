...
export function usePollRealtime({ pollId, initialPoll }: UsePollRealtimeOptions): UsePollRealtimeReturn {
  ...
  const refreshState = useCallback(async () => {
    ...
    try {
      ...
      // Fetch the single poll by its specific ID — NOT the list endpoint
      const res = await fetch(`${BASE_URL}/api/polls/${pollId}`);
      if (!res.ok) throw new Error(`Failed to fetch poll (${res.status})`);
      const json = await res.json();
      // Backend wraps response in { success, data: Poll }
      const pollData = json.data ?? json;
      if (mountedRef.current && pollData && typeof pollData === 'object' && 'id' in pollData) {
        // Ensure options is always an array to prevent iteration crashes
  ...
  useEffect(() => {
    ...
    function connect() {
      ...
      const BASE_URL = import.meta.env.VITE_API_BASE_URL || 'https://live-polling-hcl-guvi.onrender.com';
      // Correct SSE URL: /api/polls/:id/events — NOT the nonexistent /api/events
      const url = `${BASE_URL}/api/polls/${pollId}/events`;
      setConnectionState((prev) => (attemptsRef.current > 0 ? 'reconnecting' : 'connecting'));
      setError(null);

      // Use the url variable — do NOT use BASE_URL/api/events (nonexistent route → 404)
